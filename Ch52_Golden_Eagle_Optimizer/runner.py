...
def run_geo(dim=10, pop_size=50, max_iter=30):
    output_dir = os.path.dirname(os.path.abspath(__file__))
    func_classes = [getattr(opfunu.cec_based.cec2022, f"F{i}2022") for i in range(1, 13)]

    plt.figure(figsize=(12, 8))
    plt.title('Golden Eagle Optimizer (GEO) Convergence on CEC 2022 (30 Iterations)', fontsize=14)

    for func_class in func_classes:
        func_name = func_class.__name__
        print(f"Running GEO on {func_name}...")
        ...
        alg = GEO(objective_func=obj_func, bounds=b, dim=dim, pop_size=pop_size, max_iter=max_iter)
        alg.optimize()
