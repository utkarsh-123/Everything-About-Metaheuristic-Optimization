...
def run_hgso(dim=10, pop_size=50, max_iter=30):
    output_dir = os.path.dirname(os.path.abspath(__file__))
    
    func_classes = [getattr(opfunu.cec_based.cec2022, f"F{i}2022") for i in range(1, 13)]
    
    plt.figure(figsize=(12, 8))
    plt.title('HGSO Convergence on CEC 2022 Functions (30 Iterations)', fontsize=16)
    
    for func_class in func_classes:
        func_name = func_class.__name__
        print(f"Executing HGSO on {func_name}...")
        ...
        alg = HGSO(objective_func=obj_func, bounds=b, dim=dim, pop_size=pop_size, max_iter=max_iter)
        _, _ = alg.optimize()
